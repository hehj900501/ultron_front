...
const AgendarCuracion = (props) => {
	...
	const loadCuraciones = async (filterDate) => {
		const response = await findCuracionByDateAndSucursal(filterDate.getDate(), filterDate.getMonth(), filterDate.getFullYear(), sucursal, empleado.access_token);
	...
	const handleGuardarModalPagos = async (servicio) => {
		...
		await updateCuracion(servicio._id, servicio, empleado.access_token);
					...
					<Formik
						...
						validationSchema={validationSchema} >
						{
							props => <AgendarCuracionContainer
								...
								titulo={`CURACIÓN (${dateToString(filterDate.fecha_show)})`}
